...
async function getFirstLastStop(route) {
  ...
  try {
    ...
    let firstLastStop = []
    if (fullStopList.company === 'CTB') {
      firstLastStop.push(fullStopList.stopList[0])
      firstLastStop.push(fullStopList.stopList[fullStopList.stopList.length - 1])
    } else {
      firstLastStop.push(fullStopList.stopList[fullStopList.stopList.length - 1])
      firstLastStop.push(fullStopList.stopList[0])
    }
...
async function getStopIdBySeq(route, direction, seq) {
  ...
  let busStop = busStops.stopList[parseInt(seq) - 1]
  console.log(`seq: ${seq}`)
...
async function appendStopnameToStopList(input) {
  ...
  for (var i = 0; i < input.length; i++) {
    console.log('before getCitybusStopName')
    let stopnameFromDB = await getCitybusStopName(input[i].stop)
    console.log('after getCitybusStopName')
...
async function getCitybusETAByStopByStopId(stopId) {
  ...
  //convert response
  let returnMessage = ''
  for (var route of response) {
    returnMessage = returnMessage + `${route.route}
`
    for (var etaArr of route.eta) {
      returnMessage = returnMessage +
`To: ${etaArr.destination} ${etaArr.minutesLeft}
` 
    }
    returnMessage = returnMessage + `
`
  }
  console.log(`cityBus ETA by 6 digit stop ID response: ${JSON.stringify(response)}`)
  console.log(`......finish getting ETA.....`)
...
module.exports = {
  ...
  getCitybusETAByStopByRouteStopId: getCitybusETAByStopByRouteStopId,
  getCitybusETAByStopByStopId: getCitybusETAByStopByStopId
};